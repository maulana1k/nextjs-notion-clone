...
export const Navigation = () => {
    ...
    return (
        <>
            <aside ref={sidebarRef} className={cn("group/sidebar h-full w-60 bg-secondary overflow-y-auto relative flex  flex-col z-[999]", isResetting && "transition-all ease duration-300", isMobile && "w-0")} >
                ...
                <div onClick={resetWidth}
                    onMouseDown={e => handleMouseDown(e)} className="opacity-0 group-hover/sidebar:opacity-100 transition cursor-ew-resize absolute h-full w-1 bg-primary/10 right-0 top-0 " />
            </aside>
            <div ref={navbarRef} className={cn("absolute top-0 z-[999999] left-60 w-[calc(100% - 240px)]", isResetting && "transition-all ease-in-out duration-300 ", isMobile && "left-0 w-ful")} >
